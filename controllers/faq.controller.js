...
export const updateFAQ = async (req, res) => {
  ...
  if (
    !faqQuestion ||
    typeof faqQuestion !== "string" ||
    faqQuestion.trim() === ""
  ) {
    return res
      .status(400)
      .json({ message: "FAQ question must not be null or empty." });
  }

  if (!faqAnswer || typeof faqAnswer !== "string" || faqAnswer.trim() === "") {
    return res
      .status(400)
      .json({ message: "FAQ answer must not be null or empty." });
  }
...
export const deleteFAQs = async (req, res) => {
  ...
  try {
    const deletedFAQs = await deleteFAQService(faqId);
    return res
      .status(200)
      .json({ message: "FAQ(s) deleted successfully" });
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
};
